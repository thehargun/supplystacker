...
// Maps state keys to display names and data files.
// Connection strings can be added later to replace flat-file storage.
const TENANTS = {
    texas: {
        name: 'Texas',
        dataFile: 'data-texas.json',
        // connectionString: '' // to be provided later
    },
    newjersey: {
        name: 'New Jersey',
        dataFile: 'data_NJ.json',
        // connectionString: '' // to be provided later
    }
};

// Per-tenant in-memory data stores
const tenantData = {};
...
function loadTenantData(tenantId) {
    ...
    try {
        ...
        console.log(`[TENANT] ${dataFile} not found or invalid, starting fresh for ${tenantId}`);
    }
    tenantData[tenantId] = {
        users: data.users || [],
    ...
    // Load category ranks
    if (data.ItemsCategory) {
        data.ItemsCategory.forEach(cat => {
            tenantData[tenantId].categoryRanks[cat.Category] = cat.Rank;
        });
    }
    console.log(`[TENANT] Loaded data for ${tenantId} from ${dataFile}`);
}

function saveTenantData(tenantId) {
    const tenant = TENANTS[tenantId];
    if (!tenant || !tenantData[tenantId]) return;
    const store = tenantData[tenantId];
    try {
        let existingData = {};
        try {
            existingData = JSON.parse(fs.readFileSync(tenant.dataFile, 'utf8'));
        ...
        fs.writeFileSync(tenant.dataFile, JSON.stringify(dataToSave, null, 2));
    } catch (error) {
        console.error(`[TENANT] Error saving data for ${tenantId}:`, error.message);
    }
}

function saveAllTenantData() {
    Object.keys(tenantData).forEach(tenantId => saveTenantData(tenantId));
}

async function initTenantAdmins() {
...
// ==================== TENANT MIDDLEWARE ====================
// Swaps global variables to tenant-specific data when a tenant admin is logged in.
// This approach means ALL existing route code works unchanged.
const _defaultUsers = null; // will be set after loadData
const _defaultInventory = null;
const _defaultPurchases = null;
const _defaultVendors = null;
const _defaultReturns = null;
const _defaultCategoryRanks = null;

let _originalGlobals = null; // saved after initial load

function swapToTenant(tenantId) {
    if (!tenantId || tenantId === 'default') {
        // Restore defaults
        if (_originalGlobals) {
            users = _originalGlobals.users;
            inventory = _originalGlobals.inventory;
            purchases = _originalGlobals.purchases;
            vendors = _originalGlobals.vendors;
            returns = _originalGlobals.returns;
            categoryRanks = _originalGlobals.categoryRanks;
        }
        return;
    }
    const store = tenantData[tenantId];
    ...
    vendors = store.vendors;
    returns = store.returns;
    categoryRanks = store.categoryRanks;
}

app.use((req, res, next) => {
    const tenantId = (req.session && req.session.tenantId) || 'default';
    swapToTenant(tenantId);
    res.locals.tenantName = (req.session && req.session.tenantName) || null;
    res.locals.tenantId = (req.session && req.session.tenantId) || null;
    next();
});
// ==================== END TENANT MIDDLEWARE ====================

// Helper: get the correct data file path for the current request's tenant
...
function getDataFileById(tenantId) {
    if (tenantId && TENANTS[tenantId]) {
        return TENANTS[tenantId].dataFile;
    }
    // Fallback to the first tenant's data file
...
// When an item is added to one tenant, sync it to all other tenants.
function syncItemToOtherTenants(sourceTenantId, newItem) {
    for (const [tenantId, tenant] of Object.entries(TENANTS)) {
        if (tenantId === sourceTenantId) continue; // skip the source tenant
        if (!tenantData[tenantId]) loadTenantData(tenantId);
        const store = tenantData[tenantId];
        // Check if item with same name already exists to avoid duplicates
        const exists = store.inventory.some(i => i.itemName === newItem.itemName);
        if (exists) {
            console.log(`[SYNC] Item "${newItem.itemName}" already exists in ${tenantId}, skipping`);
            continue;
        }
...
// ==================== END CROSS-TENANT INVENTORY SYNC ====================
...
// Save original globals so we can swap back for default tenant
_originalGlobals = {
    users: users,
    inventory: inventory,
    purchases: purchases,
...
// Initialize all tenant data stores and create admin users
Object.keys(TENANTS).forEach(tenantId => loadTenantData(tenantId));
initTenantAdmins();
...
function saveData() {
    try {
        ...
        const dataToSave = {
            ...
            inventory: _originalGlobals ? _originalGlobals.inventory : inventory,
            purchases: _originalGlobals ? _originalGlobals.purchases : purchases,
            vendors: _originalGlobals ? _originalGlobals.vendors : vendors,
            returns: _originalGlobals ? _originalGlobals.returns : returns,
            ItemsCategory: Object.keys(_originalGlobals ? _originalGlobals.categoryRanks : categoryRanks).map(category => ({
                Category: category,
...
app.post('/login', async (req, res) => {
    ...
    // Check for tenant admin pattern: texas@supplystacker.com
    const tenantMatch = email.match(/^(\w+)@supplystacker\.com$/i);

    if (tenantMatch) {
        const stateKey = tenantMatch[1].toLowerCase();
        const tenant = TENANTS[stateKey];
        if (!tenant) {
            return res.render('login', { errorMessage: 'Unknown state location.' });
        }
        if (!tenantData[stateKey]) loadTenantData(stateKey);
        const store = tenantData[stateKey];
        const adminUser = store.users.find(u => u.role === 'admin');
        if (adminUser && await bcrypt.compare(password, adminUser.password)) {
            req.session.user = { ...adminUser };
            req.session.loggedIn = true;
            req.session.tenantId = stateKey;
    ...
    // Fallback: search default store first, then all tenant stores
    // This ensures customers added by a state admin can log in
    let foundUser = null;
    let foundTenantId = null;
    let foundTenantName = null;

    // 1. Check default data store
    const defaultUser = (_originalGlobals ? _originalGlobals.users : users).find(u => u.email === email);
    if (defaultUser) {
        foundUser = defaultUser;
        foundTenantId = null; // default tenant
        foundTenantName = null;
    }

    // 2. If not found in default, search all tenant stores
    if (!foundUser) {
        for (const [tenantId, tenant] of Object.entries(TENANTS)) {
            if (!tenantData[tenantId]) loadTenantData(tenantId);
            const store = tenantData[tenantId];
            const tenantUser = store.users.find(u => u.email === email);
            if (tenantUser) {
                foundUser = tenantUser;
                foundTenantId = tenantId;
                foundTenantName = tenant.name;
                break;
            }
        }
    }
    ...
    if (foundUser) {
        ...
        if (match) {
            ...
            req.session.loggedIn = true;
            if (foundTenantId) {
                req.session.tenantId = foundTenantId;
                req.session.tenantName = foundTenantName;
            }
...
app.post('/admin/add-inventory', multerUpload.single('image'), (req, res) => {
    ...
    const currentTenantId = (req.session && req.session.tenantId) || null;
    if (currentTenantId) {
        const addedItem = inventory[inventory.length - 1];
        syncItemToOtherTenants(currentTenantId, addedItem);
    }
...
app.get('/admin/invoices/:invoiceNumber', (req, res) => {
    ...
    // Load users and inventory from tenant-aware data file
    fs.readFile(getDataFile(req), 'utf8', (err, data) => {
        if (err) {
...
app.post('/admin/invoices/:invoiceNumber', (req, res) => {
    ...
    try {
        ...
        saveData(); // Save changes to tenant data file
...
app.get('/admin/purchases', (req, res) => {
    ...
    const data = JSON.parse(fs.readFileSync(getDataFile(req), 'utf8'));
...
app.get('/admin/purchases/:purchaseID', (req, res) => {
    ...
    let data = JSON.parse(fs.readFileSync(getDataFile(req), 'utf8'));
...
app.post('/admin/purchases/:purchaseID', (req, res) => {
    ...
    // Write the updated data back to tenant-aware data file
    fs.writeFileSync(getDataFile(req), JSON.stringify(data, null, 2), 'utf8');
...
app.post('/admin/purchases/delete/:purchaseID', (req, res) => {
    ...
    // Save to tenant-aware data file
    fs.writeFileSync(getDataFile(req), JSON.stringify(data, null, 2), 'utf8');
...
app.post('/admin/salesrep/add', (req, res) => {
    ...
    if (tenantId && tenantData[tenantId]) {
        ...
        store.salesReps.push({ id: highestId + 1, name: name.trim() });
        saveTenantData(tenantId);
    }
...
app.get('/admin/salesrep/:id/report', (req, res) => {
    ...
    // Find customers assigned to this sales rep
    const assignedCustomers = store.users.filter(u => u.role === 'customer' && u.salesRepId === repId);
    ...
    const customerReports = assignedCustomers.map(customer => {
        let total30 = 0, total90 = 0, total180 = 0;
        if (customer.invoices && customer.invoices.length > 0) {
            customer.invoices.forEach(inv => {
                const invDate = new Date(inv.dateCreated || inv.date || 0);
        ...
        return {
            id: customer.id,
            company: customer.company || customer.email,
    ...
    res.render('salesrep-report', { salesRep, customerReports });
});
